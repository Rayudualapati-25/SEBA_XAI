...
| `fabric_audit_chain` | Hands-on Hyperledger Fabric extension that prepares commitment-only audit events from SEBA-XAI policy/XAI output and provides Fabric chaincode, gateway client, and scripts for real permissioned-ledger submission after Docker/Fabric are installed. | Scaffold complete; Fabric execution pending Docker |
...
## Actual Blockchain Extension

`fabric_audit_chain` is the first real-blockchain handoff folder. It does not
change the current paper results yet because Fabric was not executed on this
machine. Docker Desktop is required before the Hyperledger Fabric test network
can be started.
